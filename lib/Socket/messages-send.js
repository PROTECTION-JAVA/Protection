...
import { createMessageFramework } from '../Utils/message-framework.js';
...
export const makeMessagesSocket = (config) => {
    ...
    // ── protection MessageFramework Init ────────────────────────────────────
    const _framework = createMessageFramework(config);

    /** Wrapper relayMessage dengan pipeline support */
    const relayMessageWithPipeline = async (jid, message, opts = {}) => {
        ...
        return _framework.runSendPipeline(
            () => relayMessage(jid, message, opts),
            ctx
        );
    };

    // Attach framework ke sock (partial) agar hook ev bisa terpasang
    try { _framework.attach(sock); } catch (_) {}
    const waitForMsgMediaUpdate = bindWaitForEvent(ev, 'messages.media-update');
    registerSocketEndHandler(async () => {
        try { await _framework.teardown(); } catch(_) {}
