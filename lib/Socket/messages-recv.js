...
import { normalizeToPipelineError } from '../Utils/message-error.js';
...
export const makeMessagesRecvSocket = (config) => {
    ...
    // ── protection Framework (dari messages-send) ───────────────────────────
    const { framework: _framework, relayMessageWithPipeline } = sock;
    ...
    // ── Framework Receive Hooks ───────────────────────────────────────────────
    if (_framework) {
        // Hook: setiap pesan masuk dicatat ke metrics
        _framework.hooks.on('message.receive', async (ctx) => {
            _framework.metrics?.inc('recv.hook.fired');
        });
    }

    /** Wrapper decode dengan pipeline support */
    const decodeNodeWithPipeline = async (decodeFn, node) => {
        if (!_framework) return decodeFn();
        const ctx = _framework.createContext({
            jid: node?.attrs?.from ?? node?.attrs?.participant ?? 'unknown',
            metadata: { nodeTag: node?.tag, nodeId: node?.attrs?.id },
        });
        return _framework.runDecodePipeline(decodeFn, ctx);
    };
    ...
    return {
        ...
        relayMessageWithPipeline,
        decodeNodeWithPipeline,
        sendMessageAck,
        sendRetryRequest,
        rejectCall,
        fetchMessageHistory,
